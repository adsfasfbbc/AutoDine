...
`catalog.json` contains fixed IDs for 20 products, 25 ingredients, and a BOM for every product. Five ingredients explicitly carry `vision_recognized: true` for edge integration fixtures. Tracked ingredients use a documented inventory snapshot at `store-main/bar`; unlimited ingredients deliberately have no inventory row.
