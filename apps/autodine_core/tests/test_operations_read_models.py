...
def test_customer_experience_summary_is_processed_recorded_and_fanned_out() -> None:
    client = _build_client()
    payload = {
        "sample_count": 42,
        "positive_ratio": 0.5,
    ...
    assert message["event_id"].startswith("outbox-")
    assert message["event_type"] == "customer.experience_summary"
    assert message["store_id"] == "store-1"
    assert message["payload"] == payload

    session = client.app.state.session_factory()
    inbox = session.get(EventInbox, "exp-summary-1")
    assert inbox is not None
    assert inbox.status is EventInboxStatus.PROCESSED
    outbox = session.scalar(select(EventOutbox).where(EventOutbox.event_type == "customer.experience_summary"))
    assert outbox is not None
    assert outbox.payload == payload
    session.close()


def test_customer_experience_summary_rejects_invalid_payload() -> None:
    client = _build_client()
    ...
    bad_ratio = client.post(
        ...
        json=_event(
            event_type="customer.experience_summary",
            event_id="exp-summary-bad-ratio",
            payload={
                "sample_count": 10,
                "positive_ratio": 1.5,
                "neutral_ratio": 0.0,
    ...
    bad_count = client.post(
        ...
        json=_event(
            event_type="customer.experience_summary",
            event_id="exp-summary-bad-count",
            payload={
                "sample_count": -1,
                "positive_ratio": 0.3,
                "neutral_ratio": 0.3,
    ...
    for response in (bad_ratio, bad_count):
        assert response.status_code == 422
        assert response.json()["code"] == "INVALID_EVENT_ENVELOPE"

    session = client.app.state.session_factory()
    assert session.get(EventInbox, "exp-summary-bad-ratio") is None
    assert session.get(EventInbox, "exp-summary-bad-count") is None
    assert session.scalar(select(EventOutbox).where(EventOutbox.event_type == "customer.experience_summary")) is None
    session.close()
