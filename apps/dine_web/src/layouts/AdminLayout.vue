...
const navItems = [
  ...
  { to: '/admin/alarms', label: '告警中心', icon: 'bell' },
...
<template>
  <div class="flex min-h-screen bg-[#f3f5f8]">
    <aside class="sticky top-0 flex h-screen w-60 shrink-0 flex-col bg-ink-900 px-4 py-6 text-white">
      <div class="px-2">
        <BrandLogo light />
      </div>

      <nav class="mt-8 flex flex-1 flex-col gap-1" aria-label="管理端导航">
        <RouterLink
          v-for="item in navItems"
          :key="item.to"
          :to="item.to"
          class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-white/60 transition-colors hover:bg-white/5 hover:text-white"
      ...
      <button
        class="flex cursor-pointer items-center gap-2 rounded-xl border border-white/10 px-3 py-2.5 text-sm text-white/60 transition-colors hover:bg-white/5 hover:text-white"
        @click="exitRole"
      >
        <svg viewBox="0 0 24 24" class="size-[18px]" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round">
